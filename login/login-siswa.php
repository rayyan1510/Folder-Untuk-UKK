...
if (isset($_POST['login'])) {
    # cek nama & nisn sudah bersih dan terhindar dari sql injection
    $nama = mysqli_real_escape_string($conn, $_POST['nama']);
    $nisn = mysqli_real_escape_string($conn, $_POST['nisn']);

    $query = mysqli_query($conn, "SELECT * FROM siswa WHERE nama='$nama' AND nisn ='$nisn'");
    $hasil = mysqli_num_rows($query);

    if ($hasil === 1) {
        session_start();
        $value = mysqli_fetch_assoc($query);
        $_SESSION['nisn'] = $value['nisn'];
                                ...
                                <div class="p-5">
                                    ...
                                    <form class="user" action="" method="post">
                                        <div class="form-group">
                                            <input type="text" class="form-control form-control-user" id="exampleInputusername" aria-describedby="usernameHelp" placeholder="Masukkan Nisn Anda" name="nisn" autocomplete="off" required>
                                        </div>
                                        <div class="form-group">
                                            <input type="password" class="form-control form-control-user" id="exampleInputnama" placeholder="Masukan Nama Anda" name="nama" autocomplete="off" required>
                                        </div>
